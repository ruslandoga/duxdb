defmodule DuxDB.GuideTest do
  ...
  describe "JSON" do
    ...
    test "load", %{conn: conn, tmp_dir: tmp_dir} do
      ...
      assert query(conn, "from read_json($json, format = 'unstructured')", %{
               "json" => birds_path
             }) ==
               [%{"duck" => [42, 43], "goose" => [4.2, 4.3]}]
    ...
    # https://duckdb.org/docs/data/json/writing_json
    @tag :tmp_dir
    test "write", %{conn: conn, tmp_dir: tmp_dir} do
      assert query(
               conn,
               "create table cities as from (values ('Amsterdam', 1), ('London', 2)) cities(name, id)"
             ) == [%{"Count" => [2]}]

      cities_path = Path.join(tmp_dir, "cities.json")
      assert query(conn, "copy cities to '#{cities_path}'") == [%{"Count" => [2]}]

      assert File.stream!(cities_path) |> Enum.map(&:json.decode/1) == [
               %{"id" => 1, "name" => "Amsterdam"},
               %{"id" => 2, "name" => "London"}
             ]
    end
